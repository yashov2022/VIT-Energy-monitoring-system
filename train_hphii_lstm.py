...
# --------------------------------------------------
query = "SELECT day_date, real_energy FROM energy_19 ORDER BY day_date"
df = pd.read_sql(query, conn)
conn.close()

if len(df) < 20:
    raise Exception("Not enough data to train model (need > 20 days)")
    exit()
...
joblib.dump(scaler, os.path.join(MODEL_DIR, "scaler_hphii.pkl"))
...
# SAVE MODEL
# --------------------------------------------------
model.save(os.path.join(MODEL_DIR, "hphii_lstm.h5"))

print("✅ hphii Model saved successfully")
