...
def load_df():
    try:
        ...
        for db in req_db:
            ...
            cursor = conn.cursor()
            cursor.execute("SHOW TABLES LIKE 'meitest17'") # Changed from meitest6 to meitest9 as in your provided code
            table = cursor.fetchone()
            if table:
                cursor.execute("SELECT * FROM meitest17")
...
def Power():
    ...
    p.vbar(x="Datetime", top=power_type, source=source, width=pd.Timedelta(seconds=55), color="#4b58a1") # Changed color
...
def Energy():
    ...
    if time_period == "per day":
        ...
        p = figure(x_axis_type="datetime", title=f"{energy_type_label.replace('EnergyWH', ' Energy in Wh').replace('EnergyVAH', ' Energy in VAh').replace('EnergyVARH', ' Energy in VARh')} per Day",
                    height=400, sizing_mode="stretch_width", tools="pan,box_zoom,reset,save")
        p.vbar(x="date", top=energy_col_df, source=source, width=pd.Timedelta(hours=23), color="#42a5f5") # Changed color
        ...
        p = figure(x_range=agg_df["month"], title=f"{energy_type_label.replace('EnergyWH', ' Energy in Wh').replace('EnergyVAH', ' Energy in VAh').replace('EnergyVARH', ' Energy in VARh')} per Month",
                    height=400, sizing_mode="stretch_width", tools="pan,box_zoom,reset,save")
        p.vbar(x="month", top=energy_col_df, width=0.8, source=source, color="#ffca28") # Changed color
...
def Voltage():
    ...
    if voltage_type == "Line Voltage":
        p.line(x="Datetime", y="LineVoltageVRY", source=source, line_width=2, color="#ef5350", legend_label="V_RY") # Changed color
        p.line(x="Datetime", y="LineVoltageVYB", source=source, line_width=2, color="#ffca28", legend_label="V_YB") # Changed color
        p.line(x="Datetime", y="LineVoltageVBR", source=source, line_width=2, color="#42a5f5", legend_label="V_BR") # Changed color
        hline1 = Span(location=11550, dimension='width', line_color='red', line_dash="dashed", line_width=2)
        hline2 = Span(location=10450, dimension='width', line_color='red', line_dash="dashed", line_width=2)
        p.add_layout(hline1)
        p.add_layout(hline2)
        ...
        p.line(x="Datetime", y="PhaseVoltageVRN", source=source, line_width=2, color="#ef5350", legend_label="V_RN") # Changed color
        p.line(x="Datetime", y="PhaseVoltageVYN", source=source, line_width=2, color="#ffca28", legend_label="V_YN") # Changed color
        p.line(x="Datetime", y="PhaseVoltageVBN", source=source, line_width=2, color="#42a5f5", legend_label="V_BN") # Changed color
        hline1 = Span(location=6668, dimension='width', line_color='red', line_dash="dashed", line_width=2)
        hline2 = Span(location=6032, dimension='width', line_color='red', line_dash="dashed", line_width=2)
        p.add_layout(hline1)
        p.add_layout(hline2)
...
def Current():
    ...
    p.line(x="Datetime", y="LineCurrentIR", source=source, line_width=2, color="#ef5350", legend_label="I_R") # Changed color
    p.line(x="Datetime", y="LineCurrentIY", source=source, line_width=2, color="#ffca28", legend_label="I_Y") # Changed color
    p.line(x="Datetime", y="LineCurrentIB", source=source, line_width=2, color="#42a5f5", legend_label="I_B") # Changed color
...
def PowerFactor():
    ...
    start_dt = pd.to_datetime(date1).normalize()
    end_dt = pd.to_datetime(date2).normalize() + pd.Timedelta(days=1) - pd.Timedelta(minutes=1)
    pf_date = df[(df["Datetime"] >= start_dt) & (df["Datetime"] <= end_dt)].copy()
    ...
    # Corrected code: Convert to pandas Series first, then use .dropna()
    unique_pfs = pf_date["PowerFactor"].unique()
    numeric_pfs = pd.Series(pd.to_numeric(unique_pfs, errors='coerce')).dropna()
    sorted_numeric_pfs = sorted(numeric_pfs, reverse=True)
    y_values = [f"{x:.2f}" for x in sorted_numeric_pfs]

    p = figure(x_axis_type="datetime", title="Power Factor per Minute",
               height=400, sizing_mode="stretch_width", tools="pan,box_zoom,reset,save",
               y_range=FactorRange(*y_values))
    
    p.scatter(x="Datetime", y="PowerFactor", size=10, source=source, color="#66bb6a", marker="circle")
...
def Frequency():
    ...
    p.vbar(x="Datetime", top="Frequency", width=pd.Timedelta(seconds=55), source=source, color="#ef5350") # Changed color
