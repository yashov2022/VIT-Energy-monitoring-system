# -*- coding: utf-8 -*-
"""
Created on Mon Mar 24 13:01:31 2025
    ...
    try:
        ...
        for db in req_db:
            conn = mysql.connector.connect(
                host="10.30.104.235",  # Corrected host name
                user="root",
                password="",
                database=db
            )
            cursor = conn.cursor()
            cursor.execute("SHOW TABLES LIKE 'meitest4'")
            table = cursor.fetchone()
            if table:
                cursor.execute("SELECT * FROM meitest4")
        ...
        # Convert columns to float, handling potential errors
...
def Energy():
    ...
    if time_period == "per day":
        agg_df = df_nonzero.groupby("date", sort=False)[energy_col].agg(
            first='first', last='last'
        ).reset_index()
        
        agg_df[energy_col] = agg_df['last'] - agg_df['first'].shift(periods=0)
        agg_df[energy_col] = agg_df[energy_col].apply(lambda x: 1000000 + x if x < 0 else x)
        
        source = ColumnDataSource(agg_df)
        p = figure(x_axis_type="datetime", title=f"{energy_type.replace('Energy', ' Energy')} per Day",
                    height=400, sizing_mode="stretch_width", tools="pan,box_zoom,reset,save")
        p.vbar(x="date", top=energy_col, source=source, width=pd.Timedelta(hours=23), color=Category10[3][1])
        hover = HoverTool(tooltips=[("Date", "@date{%Y-%m-%d}"), (f"{energy_type}", f"@{energy_col}")],
                          formatters={"@date": "datetime"})
        p.add_tools(hover)
        
    elif time_period == "per month":
        agg_df = df_nonzero.groupby("month", sort=False)[energy_col].agg(
            first='first', last='last'
        ).reset_index()
        ...
        p = figure(x_range=agg_df["month"], title=f"{energy_type.replace('Energy', ' Energy')} per Month",
                    height=400, sizing_mode="stretch_width", tools="pan,box_zoom,reset,save")
        p.vbar(x="month", top=energy_col, width=0.8, source=source, color=Category10[3][1])
        hover = HoverTool(tooltips=[("Month", "@month"), (f"{energy_type}", f"@{energy_col}")])
        p.add_tools(hover)
    
    p.xaxis.axis_label = time_period.replace("per ", "By ")
    p.yaxis.axis_label = f"{energy_col} in kWh"
    p.title.align = "center"
    st.bokeh_chart(p,use_container_width=True)
