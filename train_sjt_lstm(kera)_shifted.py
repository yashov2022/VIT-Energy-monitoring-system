...
# 🔥 FIX 1: store energy min/max
energy_min = scaler.data_min_[0]
energy_max = scaler.data_max_[0]

joblib.dump(scaler, os.path.join(MODEL_DIR, "scaler_sjt_shifted.pkl"))
...
# 🔥 FIX 2: clip predictions to valid range
pred_scaled = np.clip(pred_scaled, 0, 1)

# 🔥 FIX 3: correct inverse scaling (no dummy matrix)
predictions_wh = pred_scaled * (energy_max - energy_min) + energy_min

# 🔥 OPTIONAL (but recommended): bias correction
bias = np.mean(data[:,0]) - np.mean(predictions_wh)
predictions_wh = predictions_wh + bias
...
# SAVE MODEL
# --------------------------------------------------
model.save(os.path.join(MODEL_DIR, "sjt_lstm_shifted.keras"))
